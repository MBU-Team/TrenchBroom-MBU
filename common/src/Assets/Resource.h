...
ResourceState<T> triggerLoading(ResourceUnloaded<T> state, TaskRunnerT& taskRunner)
{
 ...
 */
template <typename T, typename TaskRunnerT>
class Resource : public ResourceBase<TaskRunnerT>
{
private:
...
public:
  ...
  {
    return std::holds_alternative<ResourceDropped>(m_state);
  }

  void process(TaskRunnerT& taskRunner) override
  {
    m_state = std::visit(
      kdl::overload(
  ...
  void upload() override
  {
    std::visit(
      kdl::overload(
        [&](ResourceLoaded<T>& state) { state.resource.upload(); }, [](const auto&) {}),
      m_state);
  }
