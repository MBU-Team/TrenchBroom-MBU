...
using ResourceT = Resource<MockResource, MockTaskRunner<MockResource>>;
using ResourceBaseT = ResourceBase<MockTaskRunner<MockResource>>;
...
TEST_CASE("ResourceManager")
{
  ...
  SECTION("addResource")
  {
    ...
    CHECK(
      ...
      == kdl::vec_static_cast<std::shared_ptr<ResourceBaseT>>(
        std::vector<std::shared_ptr<ResourceT>>{resource1}));
    ...
    CHECK(
      resourceManager.resources()
      == kdl::vec_static_cast<std::shared_ptr<ResourceBaseT>>(
        std::vector<std::shared_ptr<ResourceT>>{resource1, resource2}));
  }
  ...
  SECTION("process")
  {
    ...
    SECTION("dropping resources")
    {
      ...
      CHECK(
        resourceManager.resources()
        == kdl::vec_static_cast<std::shared_ptr<ResourceBaseT>>(
          std::vector<std::shared_ptr<ResourceT>>{sharedResources[1]}));
